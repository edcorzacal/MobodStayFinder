...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (!empty($_FILES['owner_image']['name'])) {
        ...
        $fileName = basename($_FILES['owner_image']['name']);
        ...
        if (in_array($fileType, $allowTypes)) {
            ...
            if (move_uploaded_file($_FILES['owner_image']['tmp_name'], $targetFilePath)) {
                $owner_image = $targetFilePath;
            } else {
                echo "Sorry, there was an error uploading your file.";
                $owner_image = $userData['owner_image']; // Use the existing image if upload fails
            }
        } else {
            echo "Sorry, only JPG, JPEG, PNG, & GIF files are allowed.";
            $owner_image = $userData['owner_image']; // Use the existing image if invalid format
        }
    } else {
        $owner_image = $userData['owner_image']; // Use the existing image if no new image is uploaded
    }

    // Update user data in the database
    $updateSql = "UPDATE tblownersuser SET firstname = ?, lastname = ?, middlename = ?, email = ?, gender = ?, phonenumber = ?, birthdate = ?, owner_image = ? WHERE username = ?";
    $updateStmt = $conn->prepare($updateSql);
    $updateStmt->bind_param("sssssssss", $firstname, $lastname, $middlename, $email, $gender, $phonenumber, $birthdate, $owner_image, $username);
...
<div class="container mt-5">
    ...
    <form method="POST" enctype="multipart/form-data">
        <div class="form-group text-center">
            <label for="owner_image">Profile Image</label>
            <div class="profile-image-container">
    <?php if (!empty($userData['owner_image'])) : ?>
        <img src="<?php echo htmlspecialchars($userData['owner_image']); ?>" alt="Profile Image" class="profile-image" style="width: 80px; height: 80px; border-radius: 50%; object-fit: cover;">
    <?php else : ?>
        <i class="fas fa-user-circle fa-7x"></i> <!-- Font Awesome user-circle icon -->
    <?php endif; ?>
</div>

            <input type="file" class="form-control-file mt-3" id="owner_image" name="owner_image">
        </div>
